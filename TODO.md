# ElbCar Detailing - TODO

## Pending
- [ ] **Replace logo with client's PNG** — client will provide original .png with transparent background for pixel-perfect result
- [ ] **Auto-update Instagram section via GitHub Action** — scheduled GitHub Action that fetches latest @elbcar posts and commits updated images/data to the repo. Keeps Instagram section always fresh with no manual work. Fully free, no external dependencies.
- [ ] **Add Google reviews section** — 5.0 stars, 69 reviews, include great review quote about Keramikversiegelung
- [ ] **Connect GoDaddy custom domain** — friend bought it, domain name TBD
- [ ] **Add owner photo** — Ahmet will provide later
- [ ] **Tax ID in Impressum** — placeholder [DE XXXXXXXXX] needs real value
- [ ] **More languages** — Polish, Arabic, Russian discussed

## Completed (2026-04-03)
- [x] Logo restored to original blue/silver colors with transparent background
- [x] Hero CTA buttons on both mobile and desktop: Jetzt anrufen, WhatsApp, Telegram, Termin anfragen
- [x] Removed redundant navbar Termin anfragen button
- [x] Brightened Elbe blue palette for fresher, modern feel
- [x] Tagline font weight increased for readability
- [x] Mobile hero spacing optimized (logo position, gap to tagline)
- [x] Gold background for Termin anfragen button
